...
(:~
 ...
 :
 : Requirements:
 :   - eXist-db with EXPath HTTP Client (http:send-request)
 :   - eXist-db util and xmldb modules
 ...
 :   between API requests using util:wait().
 :
 : Usage in eXist-db:
...
(: eXist-db utility module — used for sleep/delay, logging, binary handling :)
declare namespace util = "http://exist-db.org/xquery/util";

(: eXist-db XML:DB module — used for collection/resource storage :)
...
(: eXist-db collection paths :)
declare variable $app-root as xs:string := "/db/apps/hsg-annotate-data";
declare variable $config-collection as xs:string := $app-root || "/config";
declare variable $mapping-filename as xs:string := "lcsh_mapping.json";
declare variable $mapping-file as xs:string := $config-collection || "/" || $mapping-filename;
declare variable $input-taxonomy as xs:string :=
    if (doc-available($app-root || "/subject-taxonomy.xml"))
    then $app-root || "/subject-taxonomy.xml"
    else $app-root || "/subject-taxonomy-lcsh.xml";
declare variable $output-collection as xs:string := $app-root;
declare variable $output-filename as xs:string := "subject-taxonomy-lcsh.xml";
declare variable $output-taxonomy as xs:string := $output-collection || "/" || $output-filename;
...
(:~
 ...
 : Uses eXist-db's util:wait() function.
 :)
declare function local:sleep() as empty-sequence() {
    util:wait($request-delay-ms)
};
...
declare function local:query-suggest($term as xs:string) as map(*)* {
    ...
    return
        try {
            ...
            return
                if ($status = "200") then
                    ...
                    let $json-string :=
                        ...
                        else if ($body instance of xs:base64Binary) then util:binary-to-string($body, "UTF-8")
                        else string($body)
            ...
            util:log("INFO", "    API error for '" || $term || "': " || $err:description),
            ()
        }
};
...
declare function local:fetch-skos-with-retries(
    ...
    try {
        ...
        if ($attempt lt $max-retries) then (
            ...
            return (
                util:log("INFO", "    Retry " || $attempt || "/" || $max-retries || " for " || $uri || ": " || $err:description),
                (: Wait before retry :)
                util:wait($wait),
                local:fetch-skos-with-retries($skos-url, $uri, $max-retries, $attempt + 1)
            )
        ) else (
            util:log("INFO", "    SKOS error for " || $uri || ": " || $err:description),
            map { "label": (), "broader_uris": () }
        )
    }
};
...
declare function local:load-mapping() as map(*) {
    if (util:binary-doc-available($mapping-file)) then
        let $json-text := util:binary-to-string(util:binary-doc($mapping-file))
        return
...
declare function local:save-mapping($mapping as map(*)) as empty-sequence() {
    ...
    return xmldb:store($config-collection, $mapping-filename, $json-output, "application/json")[2]
};
...
declare function local:map-subjects($subjects as map(*)*) as map(*) {
    ...
    let $_ :=
        if ($existing-count gt 0) then
            util:log("INFO", "Loaded " || $existing-count || " existing mappings from " || $mapping-file)
        else ()
            ...
            function($acc, $item) {
                ...
                return
                    ...
                    if (map:contains($mapping, $ref)) then
                        ...
                        let $_ := util:log("INFO", "[" || $i || "/" || $total || "] Querying: " || $name)
                        ...
                        let $_ :=
                            if ($processed mod $save-interval eq 0) then (
                                ...
                                util:log("INFO", "  -- Progress saved: " || $i || "/" || $total || " --")
                            ) else ()
                        return map {
    ...
    let $_ := util:log("INFO", "Mapping complete: " || $result?matched || " matched, "
            ...
            function($acc, $item) {
                ...
                return
                    ...
                    if (exists($existing-bt) and $existing-bt instance of array(*) and array:size($existing-bt) gt 0) then
                        ...
                        let $done := $acc?done + 1
                        let $_ := util:log("INFO", "[" || $done || "/" || $total || "] Hierarchy: " || $data?name || " (" || $data?lcsh_uri || ")")
                        ...
                        let $_ :=
                            if (array:size($broader) gt 0) then
                                ...
                                return util:log("INFO", "    BT: " || $chain)
                            else
                                util:log("INFO", "    (no broader terms)")
                        (: Save progress periodically :)
                        let $_ :=
                            if ($done mod $save-interval eq 0) then (
                                local:save-mapping($updated-mapping),
                                util:log("INFO", "  -- Progress saved: " || $done || "/" || $total || " --")
                            ) else ()
                        return map {
                            "mapping": $updated-mapping,
                            "cache": $new-cache,
...
let $_ := util:log("INFO", "LCSH Mapper - XQuery 3.1 (eXist-db)")
let $_ := util:log("INFO", "Input taxonomy: " || $input-taxonomy)
let $_ := util:log("INFO", "Mapping file:   " || $mapping-file)
let $_ := util:log("INFO", "Output file:    " || $output-taxonomy)
let $_ := util:log("INFO", "Step: " || $step)

(: STEP 1: Map subjects to LCSH :)
let $mapping-after-step1 :=
    if ($step = ("map", "all")) then
        let $_ := util:log("INFO", string-join(for $i in 1 to 60 return "=", ""))
        let $_ := util:log("INFO", "STEP 1: Map subjects to LCSH")
        let $_ := util:log("INFO", string-join(for $i in 1 to 60 return "=", ""))
        let $subjects := local:extract-subjects($input-taxonomy)
        let $_ := util:log("INFO", "Extracted " || count($subjects) || " subjects from " || $input-taxonomy)
...
let $mapping-after-step2 :=
    if ($step = ("hierarchy", "all")) then
        let $_ := util:log("INFO", string-join(for $i in 1 to 60 return "=", ""))
        let $_ := util:log("INFO", "STEP 2: Fetch LCSH broader term hierarchies")
        let $_ := util:log("INFO", string-join(for $i in 1 to 60 return "=", ""))
        let $mapping :=
            if (exists($mapping-after-step1)) then $mapping-after-step1
            else if (util:binary-doc-available($mapping-file)) then
                parse-json(util:binary-to-string(util:binary-doc($mapping-file)))
            else
...
let $output :=
    if ($step = ("xml", "all")) then
        let $_ := util:log("INFO", string-join(for $i in 1 to 60 return "=", ""))
        let $_ := util:log("INFO", "STEP 3: Build taxonomy XML")
        let $_ := util:log("INFO", string-join(for $i in 1 to 60 return "=", ""))
        let $mapping :=
        ...
        let $_ := xmldb:store($output-collection, $output-filename, $taxonomy)
        let $_ := util:log("INFO", "Wrote taxonomy to " || $output-taxonomy)
        return $taxonomy
    else ()
