...
(:~
 : Build Subject Taxonomy using LCSH Broader Terms and HSG Topic Headings (eXist-db version)
 :
 : This is the eXist-db adapted version of ../build-taxonomy-lcsh.xq.
 : It uses eXist-db native modules (xmldb, util) instead of the EXPath file: module.
 :
 : Reads config/lcsh_mapping.json, data/document_appearances.json, config/dedup_decisions.json,
...
(: Config and data collections :)
declare variable $config-collection as xs:string := $app-root || "/config";
declare variable $data-collection as xs:string := $app-root || "/data";

(: File paths (within eXist-db collections) :)
declare variable $MAPPING-FILE := $config-collection || "/lcsh_mapping.json";
declare variable $DOC-APPEARANCES-FILE := $data-collection || "/document_appearances.json";
declare variable $DEDUP-DECISIONS-FILE := $config-collection || "/dedup_decisions.json";
declare variable $CATEGORY-OVERRIDES-FILE := $config-collection || "/category_overrides.json";
declare variable $BT-CACHE-FILE := $data-collection || "/lcsh_broader_cache.json";
declare variable $OUTPUT-FILENAME := "subject-taxonomy-lcsh.xml";
...
(:~
 : Read a JSON file from eXist-db and return its parsed content as a map/array.
 : Uses util:binary-doc() and parse-json() for JSON resources stored as binary.
 : Returns empty sequence if the resource does not exist.
 :)
declare function local:read-json-file($path as xs:string) as item()? {
    if (util:binary-doc-available($path))
    then parse-json(util:binary-to-string(util:binary-doc($path)))
    else ()
};
...
(:~
 ...
 : Uses and updates the broader-term cache stored in eXist-db.
...
(: ── Step 1: Load mapping ── :)
let $_ := util:log("INFO", "build-taxonomy-lcsh: Starting taxonomy build")
...
let $_ := util:log("INFO", "build-taxonomy-lcsh: Loaded " || map:size($mapping-loaded) || " entries from mapping")
...
let $_ := util:log("INFO", "build-taxonomy-lcsh: After dedup: " || map:size($mapping-deduped) || " entries")
...
let $mapping-final :=
    ...
    then
        ...
        (: Save updated cache to eXist-db :)
        let $cache-json := serialize($updated-cache, map { "method": "json", "indent": true() })
        let $_ := xmldb:store($data-collection, "lcsh_broader_cache.json", $cache-json, "application/json")
        let $_ := util:log("INFO", "build-taxonomy-lcsh: Updated broader term cache")
...
(: ── Step 9: Store output in eXist-db ── :)
let $serialized-xml := serialize($taxonomy-xml, map {
    "method": "xml",
    "indent": true(),
    "omit-xml-declaration": false(),
    "encoding": "UTF-8"
})
let $_ := xmldb:store($data-collection, $OUTPUT-FILENAME, $serialized-xml, "application/xml")
let $_ := util:log("INFO", "build-taxonomy-lcsh: Wrote taxonomy to " || $data-collection || "/" || $OUTPUT-FILENAME)

return
    <result>
        <message>Taxonomy written to {$data-collection}/{$OUTPUT-FILENAME}</message>
