...
(:~
 : Extract Existing Annotations from TEI XML Documents
 :
 : Replicates the logic of scripts/extract_existing_annotations.py in XQuery 3.1.
 :
 ...
 :
 : Usage with BaseX:
 :   basex -b volume-id=frus1981-88v04 extract-existing-annotations.xq
 :   basex -b volume-id=frus1981-88v04 -b base-dir=/path/to/frus-subject-taxonomy extract-existing-annotations.xq
 :
 : Usage with Saxon (EE, with EXPath file module):
 :   java -cp saxon-ee.jar net.sf.saxon.Query -q:extract-existing-annotations.xq volume-id=frus1981-88v04
 :
...
declare variable $taxonomy-path as xs:string := $base || "subject-taxonomy-lcsh.xml";
declare variable $lcsh-mapping-path as xs:string := $base || "config/lcsh_mapping.json";
declare variable $documents-dir as xs:string := $base || "data/documents/" || $volume-id || "/";

(: ── Taxonomy loading ─────────────────────────────────────────────────────── :)

(:~
 : Load subject-taxonomy-lcsh.xml into a map keyed by @ref.
 : Each entry contains term, type, count, category, subcategory, lcsh-uri, lcsh-match.
 :)
...
(:~
 ...
 : Each entry contains lcsh_uri, lcsh_label, match_quality.
 :)
declare function local:load-lcsh-mapping() as map(*) {
    if (file:exists($lcsh-mapping-path)) then
...
(:~
 : Get all text content from an element and its descendants, normalized.
 :)
...
(:~
 : Determine if a character at position $i in $text is a valid sentence boundary.
 : Checks: character is in ".!?", followed by " " and uppercase letter,
 : and is NOT a single-letter abbreviation (e.g., "U.S.") or preceded by a digit.
 :)
        ...
        if (not($ch = (".", "!", "?"))) then false()
            ...
            return
                if (not(matches($next-char, "[A-Z]"))) then false()
                    ...
                    (: Skip single-letter abbreviations like "U.S." :)
...
(:~
 : Extract sentence context around a matched term in the full text.
 : Finds the sentence containing the match by scanning for sentence boundaries.
 :
 : @param $full-text The full body text of the document
 : @param $matched-text The text that was matched
 : @param $max-chars Maximum length of context to return (default 300)
 : @return The sentence or truncated context containing the match
 :)
    ...
    if ($full-text = "" or $matched-text = "") then
        ...
        return
            if ($idx = 0) then $matched-text
                ...
                (: Find sentence start: scan backward from match position :)
                let $sent-start := local:find-sentence-start($full-text, $idx, $text-len)

                (: Find sentence end: scan forward from match end :)
                ...
                return
                    if (string-length($sentence) <= $max-chars) then $sentence
                        ...
                        (: Truncate around the middle of the match :)
...
(:~
 ...
 : Scans backward for a sentence boundary.
 :)
...
(:~
 ...
 : Scans forward for a sentence boundary.
 :)
...
(:~
 : Extract title, date, and doc_type from TEI header.
 :)
...
(:~
 : Extract all <rs> elements with @corresp from the body of a TEI document.
 : Returns a sequence of maps with ref, type, matched_text, position.
 :)
...
let $all-files := sort(file:list($documents-dir, false(), "*.xml"))

(: Process each document file :)
let $doc-results :=
    for $filename in $all-files
    let $filepath := $documents-dir || $filename
    ...
    fold-left($doc-results, map {
        ...
        return
            if (count($annotations) = 0) then
                (: No annotations - add empty document entry :)
                map:merge((
                    $state,
                    map {
                    ...
                    function($astate, $ann) {
                        ...
                        return
                            if (empty($tax-info)) then
                                (: Skip refs not found in taxonomy :)
                                map:merge((
                                    $astate,
