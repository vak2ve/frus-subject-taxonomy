...
def patch_json_file(json_path):
    ...
    try:
        ...
        print(f"\nERROR: Invalid JSON in {json_path}: {e}")
...
def main():
    ...
    for json_file in json_files:
        ...
        if has_error:
            ...
            print(f"✓ ({patch_count} documents patched)")

    # Print summary
    print("\n" + "="*60)
    print("SUMMARY - Documents patched per volume:")
    print("="*60)

    for volume_id in sorted(results.keys()):
        count = results[volume_id]
        print(f"{volume_id}: {count} documents")

    print("="*60)
    print(f"Total documents patched: {total_patched}")
