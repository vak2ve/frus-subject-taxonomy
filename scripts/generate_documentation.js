...
const volumes = [
  ...
  ["frus1981-88v01", "Soviet Union, January 1981-January 1983"],
  ["frus1981-88v03", "Soviet Union, January 1981-January 1983 (Supplement)"],
  ...
  ["frus1981-88v10", "Western Europe"],
  ["frus1981-88v11", "START I"],
  ["frus1981-88v13", "Conflict in the South Atlantic, 1981-1984"],
  ["frus1981-88v24", "Central America"],
  ["frus1981-88v38", "South Africa"],
  ["frus1981-88v41", "Global Issues II"],
  ["frus1981-88v44p1", "Global Issues: Narcotics"],
];
...
async function main() {
  ...
  children.push(bullet("Individual documents across 18 volumes"));
  ...
  children.push(para(
    "The review tool (taxonomy-review.html) is a self-contained HTML editorial interface generated by build_taxonomy_review.py. It displays all subjects organized by their assigned HSG topic categories. Editors can accept or reject LCSH mappings, reassign subjects to different categories, search across all subjects, and filter by match quality or review status. Decisions auto-save to the development server and can be exported as lcsh_decisions.json for the pipeline."
  ));
  children.push(para(
    "The annotation review tool (string-match-review.html) includes a 'Taxonomy Review' button that rebuilds the full taxonomy chain (variant groups, document appearances, taxonomy XML, and HTML) and opens the taxonomy review tool in a new browser tab, providing a seamless transition from annotation review to taxonomy review."
  ));
  ...
  children.push(para("The taxonomy is built through six phases:"));
  children.push(emptyPara());

  children.push(heading3("Phase 1: Data Extraction"));
  children.push(bullet("Split monolithic TEI volumes into per-document files (make split or split_volume.py)"));
  ...
  children.push(heading3("Phase 2: Deduplication Review"));
  ...
  children.push(heading3("Phase 3: Annotation Review"));
  children.push(bullet("Run string-match annotation against volumes (annotate_documents.py)"));
  children.push(bullet("Review annotation matches in string-match-review.html (accept/reject per volume)"));
  children.push(bullet("Run Pipeline button applies decisions, rebuilds variant groups, and propagates to taxonomy"));
  children.push(bullet("Taxonomy Review button rebuilds and opens taxonomy-review.html in a new tab"));

  children.push(heading3("Phase 4: Taxonomy Build"));
  ...
  children.push(heading3("Phase 5: Taxonomy Review"));
  children.push(bullet("Review LCSH mappings in taxonomy-review.html (accept/reject)"));
  children.push(bullet("Reassign subjects to different categories and subcategories"));
  children.push(bullet("Export decisions as lcsh_decisions.json; auto-saves to server"));

  children.push(heading3("Phase 6: Mockup and Validation"));
  ...
  children.push(bullet("Run validate_data.py to check integrity across all volumes"));
  ...
  const scriptRows = [
    ...
    new TableRow({ children: [dataCell("build_taxonomy_review.py", 3200, { bold: true }), dataCell("Generates taxonomy-review.html from taxonomy XML and config files", 6160)] }),
    new TableRow({ children: [dataCell("rebuild_taxonomy_review.py", 3200, { bold: true }), dataCell("Full taxonomy rebuild chain: variant groups → doc appearances → XML → HTML", 6160)] }),
    new TableRow({ children: [dataCell("build_annotation_review.py", 3200, { bold: true }), dataCell("Builds string-match-review.html with pipeline action buttons", 6160)] }),
    new TableRow({ children: [dataCell("run_reviewed_pipeline.py", 3200, { bold: true }), dataCell("Orchestrates the full 8-step post-review pipeline for a volume", 6160)] }),
    new TableRow({ children: [dataCell("annotate_documents.py", 3200, { bold: true }), dataCell("String-match annotation engine with stoplist and variant support", 6160)] }),
    ...
    new TableRow({ children: [dataCell("build_variant_groups.py", 3200, { bold: true }), dataCell("Consolidates variant name forms from dedup, LCSH, and manual overrides", 6160)] }),
    new TableRow({ children: [dataCell("split_volume.py", 3200, { bold: true }), dataCell("Splits a monolithic TEI volume into per-document files", 6160)] }),
    new TableRow({ children: [dataCell("validate_data.py", 3200, { bold: true }), dataCell("Validates data integrity across all volumes", 6160)] }),
    ...
    new TableRow({ children: [dataCell("apply_review_decisions.py", 3200, { bold: true }), dataCell("Applies exported review decisions (rejections, merges, LCSH) to config", 6160)] }),
  ];
  children.push(new Table({
    width: { size: CONTENT_W, type: WidthType.DXA },
  ...
  const dataRows = [
    ...
    new TableRow({ children: [dataCell("category_overrides.json", 3600, { bold: true }), dataCell("Manual category reassignments from editorial review (updated by taxonomy-review.html)", 5760)] }),
    new TableRow({ children: [dataCell("variant_overrides.json", 3600, { bold: true }), dataCell("Manual variant group split/merge overrides", 5760)] }),
    new TableRow({ children: [dataCell("annotation_stoplist.json", 3600, { bold: true }), dataCell("Terms excluded from string matching (too generic)", 5760)] }),
  ...
  children.push(para("A Makefile provides the single entry point for all pipeline operations. To regenerate from current data:"));
  children.push(emptyPara());
  const cmds = [
    "# One-time setup (install deps + split + convert + build tools)",
    "make setup",
    "",
    "# Start the development server",
    "make serve",
    "",
    "# Build review tools individually",
    "make review           # string-match-review.html",
    "make taxonomy-review  # taxonomy-review.html",
    "",
    ...
    "python3 build_taxonomy_lcsh.py all   # Full LCSH fetch + build",
    "python3 build_taxonomy_lcsh.py build # XML only (cached terms)",
    "python3 generate_mockup_data.py      # Mockup data JSON",
    "python3 build_mockup_html.py         # Mockup HTML",
  ];
