...
The current proof-of-concept covers 18 volumes from the 1969--1988 subseries. The full FRUS corpus comprises 538 volumes.
...
- **string-match-review.html** -- Review annotation matches per volume; includes pipeline action buttons (Validate, Run Pipeline, Rebuild, Taxonomy Review)
- **taxonomy-review.html** -- Review taxonomy organization, accept/reject LCSH mappings, reassign categories; auto-saves decisions to the server
...
Review decisions made in the annotation review tool reach downstream tools through these paths:

1. **Taxonomy Review Tool** (via pipeline rebuild): After reviewing annotations, click **Run Pipeline** in the annotation review tool, then click **Taxonomy Review →**. The pipeline applies your decisions to the config files, rebuilds the taxonomy XML, and generates a fresh taxonomy-review.html incorporating all changes. Taxonomy review decisions (LCSH accept/reject, category reassignments) auto-save to the server.
...
3. **Taxonomy XML + Annotated Volumes** (run pipeline): Click **Run Pipeline** in the annotation review tool, or run `make pipeline VOL=<volume-id>` from the command line.
...
| `/api/save-taxonomy-decisions` | POST | Save LCSH + category decisions |
