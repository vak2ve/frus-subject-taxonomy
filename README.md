...
All scripts are in the `scripts/` directory. Python 3.8+ is required; no additional packages are needed.
...
python3 scripts/extract_existing_annotations.py frus1969-76v19p2

# Build the taxonomy XML
python3 scripts/build_taxonomy_lcsh.py build

# Generate and build the mockup
python3 scripts/generate_mockup_data.py
python3 scripts/build_mockup_html.py

# Run the full post-review pipeline for a volume
python3 scripts/run_reviewed_pipeline.py frus1969-76v19p2
```
...
The `data/documents/` directory lives one level above the repo root and is also used by `apply_curated_annotations.py`, `extract_existing_annotations.py`, and `merge_annotations.py`.
...
This produces `string_match_results_<volume-id>.json` in the repo root, containing all matches with sentence context, positions, and term metadata.

### 5. Review annotations

```bash
# Build the review tool (if not already built)
python3 scripts/build_annotation_review.py

# Serve locally
python3 -m http.server 9090
```

Open `string-match-review.html` in a browser, review and accept/reject matches, then export decisions as `config/annotation_rejections_<volume-id>.json`.

### 6. Run the post-review pipeline

```bash
python3 scripts/run_reviewed_pipeline.py <volume-id>
```
...
3. **Taxonomy XML + Annotated Volumes** (run pipeline): Export decisions, place the JSON in `config/` as `annotation_rejections_<volume-id>.json`, then run `python3 scripts/run_reviewed_pipeline.py <volume-id>`.
